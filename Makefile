...
build:
	...
	go build -o metad -ldflags="\
    	-X 'github.com/vishnushankarsg/metad/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/vishnushankarsg/metad/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/vishnushankarsg/metad/versioning.Branch=$(BRANCH)'\
		-X 'github.com/vishnushankarsg/metad/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e-polybft:
	...
	go build -o artifacts/metad .
	env EDGE_BINARY=${PWD}/artifacts/metad E2E_TESTS=true E2E_LOGS=true \
...
test-property-polybft:
    # We can not build with race because of a bug in boltdb dependency
	go build -o artifacts/metad .
	env EDGE_BINARY=${PWD}/artifacts/metad E2E_TESTS=true E2E_LOGS=true go test -v -timeout=30m ./e2e-polybft/property/...
